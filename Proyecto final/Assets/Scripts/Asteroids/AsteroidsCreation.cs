...
public class AsteroidsCreation : MonoBehaviour
{
        ...
        for (int x = -areaSideSize + 2; x < 0; x++)
        {
            for (int y = -areaSideSize + 2; y < areaSideSize; y++)
            {
                for (int z = -areaSideSize + 2; z < 0; z++)
                {
                    ...
                    asteroids[asteroidIndex].GetComponent<Rigidbody>().constraints = RigidbodyConstraints.FreezeAll;
    ...
    private int AsteroidOppositeDirection(Axis axis, GameObject asteroid)
    {
    ...
    private int GetSpaceshipDirection(Axis axis)
    {
        switch (axis)
        {
            case Axis.xAxis:
